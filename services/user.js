  ...
  app.get(
    ...
    async function(req, res) {
      ...
      if (
        ...
        res
          .status(200)
          .send({ ...user, statusName: "รอคำตอบรับจากคุณ" });
      } else if (
        requestFromUser &&
        requestFromUser.id == friendId &&
        requestFromUser.status == "friend"
      ) {
        res.status(200).send({ ...user, statusName: "Friend" });
      } else if (
        requestToUser &&
        ...
        res
          .status(200)
          .send({ ...user, statusName: "่ส่งคำขอเป็นเพื่อนแล้ว" });
      } else if (
        requestToUser &&
        requestToUser.id == friendId &&
        ...
        res.status(404).send({ statusName: "User not found" });
      }
    }
  );
};
